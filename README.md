<p align="center">
  ...
  <p align="center">SPAR Fall 2026 · <a href="https://github.com/alanasung/influence-knowledge-locate">Locating Factual Knowledge With Influence Functions</a></p>
</p>
...
**Locating Factual Knowledge With Influence Functions**. The shared engineering spine (config, caching, hooks, metrics,
...
| Stage | Module | Output |
|---|---|---|
| Compose config | `configs/` + `infloc.configs` | resolved `config.yaml` |
| Build data | `infloc.data` | splits + manifest |
| Load model | `infloc.models` | `LoadedModel` + resolved commit |
| Run stages | `scripts/run_experiment.py` | per-stage JSON |
| Aggregate | `infloc.reporting` | `results.json` + tables + figures |
...
├── src/infloc/       # installable library (print-free)
...
- SPAR Fall 2026 project posting: https://github.com/alanasung/influence-knowledge-locate
- Complexity bar: Critical Data PRIMED-AI / RecursiveJEPA engineering standard

## Citation

```bibtex
@misc{infloc_spar_f26,
  title        = {Locating Factual Knowledge With Influence Functions},
  author       = {Alana Sung},
  year         = {2026},
  howpublished = {SPAR Fall 2026},
  note         = {Use influence functions on tiny models to find which training points most affect factual behaviors.},
  url          = {https://github.com/alanasung/influence-knowledge-locate}
}
```
...
Ablations live under `src/infloc/ablation/` and return structured dicts. Presets
