# Design Document: Locating Factual Knowledge With Influence Functions

## 1. Purpose

This document is the method contract for **Locating Factual Knowledge With Influence Functions**. It specifies what is
...
One-liner: Use influence functions on tiny models to find which training points most affect factual behaviors.

Hypothesis: Use influence functions on tiny models to find which training points most affect factual behaviors.
...
|---|---|---|
| Config schema | `infloc.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `infloc.configs.loader` | Hydra compose API |
| Device | `infloc.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `infloc.models.registry` | Pinned revisions + hardware fit |
| Hooks | `infloc.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `infloc.models.generation` | Chat template path recording |
| Data | `infloc.data` | Synthetic items, splits, manifests |
| Evaluation | `infloc.evaluation` | Metrics, failure harness, protocols |
| Ablation | `infloc.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `infloc.reporting` | Aggregate → MD/TeX/figures |
| Cache | `infloc.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/influence-knowledge-locate
...
`src/infloc/infloc/` are preserved across regeneration.
